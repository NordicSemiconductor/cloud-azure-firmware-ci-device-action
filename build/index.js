/******/ ;(() => {
	...
	;(() => {
		...
		const getRequiredInput = (input) =>
			(0, core_1.getInput)(input, { required: true }).trim()
		...
		const timeoutInMinutes = parseInt(
			getRequiredInput('timeout in minutes'),
			10,
		)
		const hexFile = getRequiredInput('hex file')
		const fotaFile = getRequiredInput('fota file')
		const multilineOrUndefined = (s) =>
			s.length > 0 ? s.split('\n') : undefined
		const abortOn = multilineOrUndefined((0, core_1.getInput)('abort on'))
		const endOn = multilineOrUndefined((0, core_1.getInput)('end on'))
		const endOnWaitSeconds = parseInt(
			(0, core_1.getInput)('end on waitSeconds'),
			10,
		)
		...
		const run = async () => {
			tries--
			numTry++
			const p = (0, child_process_1.spawn)(
				'npm',
				[
					'exec',
					'--',
			...
			p.on('close', (code) => {
				clearTimeout(t)
				;(0, core_1.setOutput)('connected', code === 0)
				if (timedOut) {
					console.error('Timed out.')
					;(0, core_1.setOutput)('timeout', true)
					;(0, core_1.setOutput)('try', numTry)
					process.exit(-108)
				}
